...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    if (labelFormatter) {
      return <div className={cn("font-medium", labelClassName)}>{labelFormatter(computed, safePayload)}</div>
    }
                  ...
                  >
                    <div className="grid gap-1.5">
                      ...
                      <span className="text-muted-foreground">
                        {itemConfig?.label ?? (item as any)?.name ?? String(key)}
                      </span>
                    </div>
      ...
      {payload.map((item, idx) => {
        ...
        return (
          ...
          >
            ...
            {itemConfig?.label ?? (item as any)?.value ?? String(key)}
          </div>
        )
      })}
    </div>
